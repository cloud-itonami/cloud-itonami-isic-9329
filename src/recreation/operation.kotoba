(ns recreation.operation
  "OperationActor -- one recreation-venue operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (RecOps-LLM) is sealed into a single node (:advise); its proposal is
  ALWAYS routed through the Recreation Safety Governor (:govern) and
  ...
  One graph run = one recreation-venue operation (intake -> advise ->
  ...
  decision to a human operator (licensed venue operator). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/resume-operation` ALWAYS reaches this node when the
  governor is clean -- see `recreation.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [recreation.recreationopsllm :as recreationopsllm]
...
(defn build
  ...
  `recreation.store/Store`).
  opts:
    :advisor      -- a `recreation.recreationopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (recreationopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; RecOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (recreationopsllm/-advise advisor store request)]
            {:proposal p :audit [(recreationopsllm/trace request p)]})))

      ;; Recreation Safety Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
